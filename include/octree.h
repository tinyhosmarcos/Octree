...
class Octree{
  ...
  bool slip(Point<T> point, Node<T> *node){
  ...
  bool find(Point<T> val, Node<T>* &p, Node<T>* &par, Node<T>* start = nullptr){
    ...
    for( ; p; par = p, p = next){
      next = nullptr;
      for(int i = 0; i < 8; ++i){
        if(p->child[i] && slip(val, p->child[i])) next = p->child[i];
      }
      if(!next){
 ...
 public:
  ...
  bool find(Point<T> point){
    Node<T>* tmp, *tmp2;
    return (slip(point, root) && find(point, tmp, tmp2));
  }

  void insert(Point<T> point, Node<T> *head = nullptr){
    Node<T> *pos, *tmp;
    if(slip(point, root) && !find(point, pos, tmp, head)){
  ...
  void erase(Point<T> point){
    Node<T> *pos, *parent;
    if(slip(point, root) && find(point, pos, parent)){
